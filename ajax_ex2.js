...
$.ajax({
	...
	success: function(data) {$.each(data, function(i, item) {
		$table_row.replaceWith('<tr id = "table_row"><td>' + item.measure_category +'</td><td>' + item._2010 +'</td><td>' + item._2011 + '</td><td>' + item._2012 + '</td><td>' + item._2013 + '</td><td>' + item._2014 + '</td></tr>' );
		})
	console.log(data);
	}
});
